...
function filteredDocs() {
  ...
  if (terms.length)
    list = list.filter((d) => {
      const hay = [d.title, d.author, d.description, ...d.tags, d.searchText]
        .join(" ")
        .toLocaleLowerCase();
...
function renderLibrary() {
  ...
  document.querySelector("#breadcrumb-current").textContent = titles[view];
...
function renderRows() {
  ...
  document.querySelector("#list-summary").textContent = query
    ? `找到 ${list.length} 份相关资料`
    : `共 ${list.length} 份资料 · 点击标题即可阅读`;
  container.innerHTML = list.length
    ? `<table class="document-table"><thead><tr><th scope="col">资料名称</th><th scope="col">分类与标签</th><th scope="col">大小</th><th scope="col">更新日期</th><th scope="col"><span class="sr-only">操作</span></th></tr></thead><tbody>${list
        .map(
          (d) =>
            `<tr><td><div class="document-name">${badge(d)}<div><a class="document-title" href="#/read/${d.id}">${escapeHTML(d.title)}${record(d.id).completed ? `<span class="completed-check" title="已读完">${icon("check")}</span>` : ""}</a><p>${escapeHTML(d.author)}<span>·</span>${d.sample ? "示例文档" : escapeHTML(d.description)}</p></div></div></td><td><div class="tags"><span class="category-tag category-${d.category}">${d.category}</span>${d.tags
              .slice(0, 1)
        ...
        .join("")}</tbody></table>`
    : `<div class="empty-state">${icon(query ? "search" : view === "favorites" ? "star" : "book")}<h3>${query ? "还没有找到匹配的资料" : view === "favorites" ? "把喜欢的内容留在这里" : view === "recent" ? "阅读，从打开一份资料开始" : "这个分类还没有资料"}</h3><p>${query ? "试试更短的关键词，或切换文件类型。" : view === "favorites" ? "点击资料旁的星形图标，就能加入收藏。" : "回到书架，挑选一份想读的内容。"}</p><button id="reset-filters" class="read-button">${query ? "清除筛选" : "浏览全部资料"}${icon("next")}</button></div>`;
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Escape") {
    document.body.classList.remove("nav-open");
    if (document.body.classList.contains("reading")) location.hash = "/";
  }
});

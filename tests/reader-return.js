async (page) => {
  ...
  const expectedTotal = Number(await page.locator('[data-view="all"] small').textContent());
  for (let visit = 0; visit < 2; visit++) {
    await page.getByRole("link", {
      name: "Attention Is All You Need（Transformer 原始论文）",
      exact: true,
    }).click();
    await page.waitForFunction(() => document.querySelector("#pdf-total")?.textContent === "15");
    ...
    const layout = await page.evaluate(() => ({
      ...
      listTop: document.querySelector(".document-table").getBoundingClientRect().top,
      sidebarPosition: getComputedStyle(document.querySelector('[aria-label="主导航"]')).position,
      inert: document.querySelector(".workspace").inert,
      readerOpen: !!document.querySelector(".reader"),
      viewportHeight: innerHeight,
    }));
    if (
      layout.rows !== expectedPapers || layout.sidebarPosition !== "fixed" ||
      layout.topbarTop > 100 || layout.listTop >= layout.viewportHeight ||
      layout.inert || layout.readerOpen
    ) throw new Error("RETURN_BOOKSHELF_LAYOUT_BROKEN " + JSON.stringify(layout));
  }
  ...
  await page.getByRole("link", { name: "返回书架", exact: true }).click();
  if (await page.locator(".document-table tbody tr").count() !== expectedTotal)
  ...
  if (mobile.top > 100 || mobile.listTop >= mobile.height || mobile.width > mobile.viewport)

...
[第 11 篇](11-Java%20面试专题：并发、排障与工程取舍.md)另有七个 Java 21 实验，覆盖线程池接纳、Future 超时、同池等待、ThreadLocal、原子性与虚拟线程准入。[第 13 篇](13-Oracle%20迁移%20PostgreSQL：语义核对、切换与回退方案.md)的独立 PG 实验验证了空值、精度、时间、序列、冲突、分项目对账与失败事务。

其中“唯一键异常后继续查询”值得优先看：PG 显式事务发生错误后，需要正确结束失败状态，不能把 H2 的通过结果直接当作目标库保证。这组 PG 实验尚未接入当前 ExportStore，也没有验证 Spring/JDBC 并发事务。它提供了迁移适配时应补的测试方向。

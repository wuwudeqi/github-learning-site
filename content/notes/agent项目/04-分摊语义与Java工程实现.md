...
title: Agent 项目｜一笔费用怎样分摊：Java 工程实现
author: 个人整理
category: 笔记
date: "2026-09-09"
...
标准指标通过字段枚举映射 SQL 标识符，过滤值使用参数绑定。表名、排序方向、分组字段不能直接拼模型字符串；参数化也不能替代这些标识符校验。受限 Text-to-SQL 的详细过程见[第 05 篇](05-可信查询与Text-to-SQL评测.md)。
...
查询重试、HTTP 重试、图节点重试还要共用总预算，避免三层各试三次造成请求放大。权限错误不重试；副作用重试不换操作 ID。完整故障复现和验证证据放在[第 08 篇](08-难点排查与验证实验.md)，这里重点掌握机制。
...
下一篇：[让 Text-to-SQL 查对数](05-可信查询与Text-to-SQL评测.md)。

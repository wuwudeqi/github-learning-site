...
具体公式、不同规则比较、分币和联合分摊反例，统一看[分摊与 Java 实现](04-一笔费用怎样分摊，Java%20又该守住什么.md)。
...
首版先用一个 Worker，重启确认旧进程退出后恢复。以后加多个 Worker，还得处理旧进程失去租约却继续写结果的问题；仅仅把任务行锁住，并不能让旧进程自动停止。具体处理见[执行流程](02-Agent%20怎么一步步完成一次汇报.md)与[难点实验](08-数字不对、口径串了、任务重复：怎么一步步查.md)。
...
接着读：[Agent 如何一步步执行](02-Agent%20怎么一步步完成一次汇报.md)。

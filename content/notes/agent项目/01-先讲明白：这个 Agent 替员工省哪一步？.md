...
Java 面试可以沿这里继续展开：API 与后台任务怎样分配线程，外部调用超时后资源是否释放，事务边界如何避免长时间占用连接。[第 11 篇](11-Java%20面试专题：并发、排障与工程取舍.md)使用普通后端场景独立解释这些问题。已有系统采用 JDK 21 和 PostgreSQL 时，其升级与迁移的实施判断另见第 12、13 篇，不把运行在新版本上等同于已经验证性能收益。

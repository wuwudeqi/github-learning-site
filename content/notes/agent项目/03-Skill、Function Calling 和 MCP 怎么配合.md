...
验证时在 Java 提交之后主动丢弃响应，重启 Worker 并重试；结果应仍指向原任务。相关局部幂等实验见[难点排查与验证](08-数字不对、口径串了、任务重复：怎么一步步查.md)，它不能替代完整 MCP 跨进程测试。
...
继续阅读：[分摊语义与 Java 工程实现](04-一笔费用怎样分摊，Java%20又该守住什么.md)、[数据权限](06-用户说“导出全部”，怎样守住数据权限.md)和[多格式汇报](07-图表、Excel%20和查询数据，怎样做到对得上.md)。

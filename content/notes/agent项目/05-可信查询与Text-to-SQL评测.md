...
title: Agent 项目｜Text-to-SQL 怎样从能运行变成查对数
category: 笔记
author: 个人整理
date: "2026-09-09"
...
这里的 `authorized_allocated_costs` 是执行器按有效计划提供的关系，已限制项目、目标、轴、规则、发布版本、币种和指标口径。名字只是示意；真正数据库隔离需按[第 06 篇](06-数据权限与导出安全.md)实现验证。不能把这段 SQL 放到全库账号下，就认为权限已经解决。
...
回归至少加三种变化：换成投资轴；预算与实际采用不同批准规则；用户只允许看 D_A。它们能检查修复是否覆盖机制，而非记住一句测试原文。完整事故叙述和证据组织见[第 08 篇](08-难点排查与验证实验.md)。
...
下一篇：[数据权限怎样管住查询与导出](06-数据权限与导出安全.md)。

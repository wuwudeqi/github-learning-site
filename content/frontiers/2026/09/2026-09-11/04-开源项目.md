...
[![Paperless 的输出是可检索档案，GROBID 的输出是结构化论文 TEI XML](images/project-document-outputs.svg)](images/project-document-outputs.svg)

图：左边解决归档和搜索，右边提取学术字段，两条路线可分别使用。本站依据 [Paperless 文档](https://docs.paperless-ngx.com/)和 [GROBID 仓库](https://github.com/grobidOrg/grobid)绘制输出对照；纸张与字段为示意，没有运行解析。
...
[![同一依赖问题可以检查检索段落，也可以沿实体关系定位相关原文](images/project-rag-evidence.svg)](images/project-rag-evidence.svg)

图：A→B→C 的材料为虚构示例，说明为什么关系边必须能回到原始文档。本站依据 [Haystack](https://github.com/deepset-ai/haystack)与 [LightRAG](https://github.com/HKUDS/LightRAG)文档绘制；Haystack 可扩展多跳或图检索，这不是两者能力的互斥划分，也未作效果比较。
...
[![模型加载分出 MODEL、CLIP、VAE，提示编码后参与采样，潜表示经解码成为图片](images/project-comfy-flow.svg)](images/project-comfy-flow.svg)

图：重点看 KSampler 前后的数据类型：输入有模型、提示条件和初始潜表示，输出仍是潜表示；VAE Decode 才把它变成像素。本站按[官方 SD 1.5 文生图教程](https://docs.comfy.org/tutorials/basic/text-to-image)和 [KSampler 说明](https://docs.comfy.org/built-in-nodes/sampling/ksampler)绘制简图，合并了正负提示编码节点的显示，连线不表示同一条条件输入。本文未运行 ComfyUI。
...
[![从错答追到生成和检索步骤，再把失败案例加入固定评测集](images/project-opik-trace.svg)](images/project-opik-trace.svg)

图：缺少文档 2 是一个虚构的排查例子；应回看实际证据再确认原因。本站依据 [Opik 文档](https://www.comet.com/docs/opik/)绘制概念调用链，没有真实日志、产品截图或自动评分实测。最下方固定 id 的做法可继续接本期技术实践。

...
- 桌面与手机书架，论文 / 书籍 / 笔记按真实文件夹逐层浏览，支持面包屑、可分享的目录链接、文件类型筛选与自然排序。
...
网站根据 `content/` 中的文档路径展示目录；顶层 `papers`、`books`、`notes` 分别显示为论文、书籍、笔记。将章节放在 `content/notes/transformer/` 后，网站就会显示 transformer 文件夹。默认文件夹在前，文档按真实文件名自然排序；显示文档标题及文件名。

只有已收录的 Markdown、PDF、EPUB 会生成文件与文件夹入口。图片、代码、元数据和没有可阅读文档的目录不会出现在列表中，附件仍随文档发布。建议文档的 category 与顶层目录对应。

搜索在当前分类的所有子文件夹中查找，书架搜索覆盖全部资料，结果显示可点击的所在目录。收藏和最近阅读保留为文档快捷列表。目录地址支持刷新、前进后退与分享；阅读后返回原列表，直接打开文档链接时返回其所在目录。文档 id 和本地阅读记录格式保持不变。

Markdown 中可以直接写 `[第一章](01-必要基础.md)`，链接会按当前文件所在目录解析。目标为本站已收录文档时，在当前页面打开对应阅读器；支持中文文件名、跨目录路径，以及 Markdown 标题锚点（如 `01-必要基础.md#本章完成标准`）。外部链接和未收录的附件仍按普通链接打开，带 `download` 的链接保留下载用途。

## 前沿自动编辑

“前沿”位于分类首位，日期文件夹倒序排列，每期五块：AI 动态（每日 15 条，优先大厂重要发布与有热度证据的 AI 项目；5 条重点详解 + 10 条速读）、开源模型、论文精选、开源项目（每日 10 个应用层工具与实用方法，来源多样）、技术实践。重点新闻每条约 500–800 字，解释变化、原理、用途和开放范围；开源项目介绍用途、上手路径、许可及使用条件。详细来源、图文、专题跟踪和论文归档规则见 [编辑规则](docs/frontiers-editorial.md)。北京时间 07:30 启动，目标 08:30 前完成上线供阅读。自动整理在本地 Mac 上由 Codex 调度；仓库的 GitHub Actions 负责构建与发布，不负责定时 AI 采集。

每期先记录官方来源覆盖及候选取舍，再写正文。`npm run check:frontiers -- --date YYYY-MM-DD` 要求目标日期的六个文档齐全，并检查 15 条新闻、10 个开源项目及对应证据；不传日期时检查已有期刊。它检查记录与成稿的一致性，事实与重要程度仍须编辑核验。发布后用 `node scripts/verify-frontiers-published.mjs --date YYYY-MM-DD` 核对线上当期文件。08:30、09:30 增加补漏检查，持久进度及上下文恢复方式见 [执行手册](docs/frontiers-runbook.md)。
